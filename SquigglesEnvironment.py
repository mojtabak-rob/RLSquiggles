...
class SquigglesEnvironment(py_environment.PyEnvironment):
    ...
    def _reset(self):
        ...
        self._time_since_second_real_play = 0
        self._episode_ended = False
        return ts.restart(np.array([self._time_since_real_play, self._time_since_second_real_play], dtype=np.int32))
    ...
    def _step(self, action):
        ...
        if action == 1 or action == 0:
            self._state += 1
            self._time_since_second_real_play += 1
            self._time_since_real_play += 1
        ...
        if self._state >= 10000:
            ...
            return ts.transition(
                np.array([self._time_since_real_play, self._time_since_second_real_play], dtype=np.int32), reward=reward, discount=1.0)
